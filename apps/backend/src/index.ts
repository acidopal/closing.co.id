		...
		cors({
			origin: (request) => {
				...
				const allowedOrigins = [
					...envFrontendUrls,
					'https://app.scalebiz.chat',
					'https://scalebiz.chat',
					'https://local-fe.scalebiz.chat',
					'https://local-api.scalebiz.chat',
					'https://local-n8n.scalebiz.chat',
					'http://localhost:5173',
					'http://localhost:3000',
					'http://localhost:3005',
					'http://localhost:3006',
	...
	.group('/api', (app) =>
		app
			...
			.use(customer)
				...
				.group('/instagram-channels', (app) => app.use(instagram))
				.group('/tiktok-channels', (app) => app.use(tiktok))
				.use(webhook)
...
if (IS_API_MODE) {
	...
	console.log(
		`🚀 Scalebiz API v2.0 running at http://localhost:${app.server?.port}`,
	)
