...
const ENABLE_INBOX_FLOW_RUNTIME = ['1', 'true', 'yes', 'on'].includes(
	String(process.env.ENABLE_INBOX_FLOW_RUNTIME || '').toLowerCase(),
)
...
function resolveConfiguredChatbotId(config: Record<string, unknown>): string | null {
	return (
...
function extractPreferredDivisionName(statusTimelineTexts: string[]): string | null {
	for (const text of statusTimelineTexts) {
...
function isAiVsHumanIdentityIntent(incomingText: string): boolean {
...
function isAiInternalOrModelQuestion(incomingText: string): boolean {
	const normalized = String(incomingText || '').toLowerCase()
	if (!normalized.trim()) return false

	const hasInternalKeyword =
		/\b(model|ai model|pakai model|gpt|chatgpt|llm|system internal|sistem internal|mesin ai)\b/.test(
			normalized,
		)
	const hasQuestionPattern =
		/[?]/.test(normalized) ||
		/\b(apa|apa ya|apa sih|yang dipakai|yang digunakan|kamu pakai|versi|pakai)\b/.test(
			normalized,
		)
	return hasInternalKeyword && hasQuestionPattern
}
...
function isLikelyEscalationIntent(
	...
	if (isAiVsHumanIdentityIntent(incomingText)) {
		return true
	}

	if (
		/\b(agent|human|admin|customer service|cs|tim|staff|orang)\b/.test(normalized) &&
		/\b(hubung|sambung|transfer|eskalas|escalat|talk|speak|minta)\b/.test(
			normalized,
		)
	) {
		return true
	...
	if (
		/\b(komplain|complaint|marah|kecewa|refund|gagal|error|darurat|urgent|urgent)\b/.test(
			normalized,
		)
	) {
		return true
	...
	const hasBookingIntent =
		/\b(book|booking|jadwal|schedule|reserv|appointment|visit|kunjung|datang|slot)\b/.test(
			normalized,
		)
	const hasLocationOrBranch =
		/\b(cabang|branch|lokasi|location)\b/.test(normalized)
	const hasTimeHint =
		/\b(senin|selasa|rabu|kamis|jumat|sabtu|minggu|monday|tuesday|wednesday|thursday|friday|saturday|sunday|jam|pukul)\b/.test(
			normalized,
		) || /\b\d{1,2}[:.]\d{2}\b/.test(normalized)
	const hasRelativeDateHint =
		/\b(hari\s+(senin|selasa|rabu|kamis|jumat|sabtu|minggu)|hari ini|hari besok|besok|lusa|minggu ini|this week|today|tomorrow)\b/.test(
			normalized,
		)

	if (hasBookingIntent && (hasLocationOrBranch || hasTimeHint)) {
		return true
	}
	if (hasLocationOrBranch && hasTimeHint && hasRelativeDateHint) {
		return true
	}
	...
	if (
		...
		(hasBookingIntent || hasLocationOrBranch)
	) {
		return true
	}
...
function isMetaProtectedMediaUrl(url: string): boolean {
	return url.includes('lookaside.fbsbx.com') || url.includes('graph.facebook.com')
}
...
function resolveAutoReplyDebounceDelayMs(burstCount: number): number {
	const [minSeconds, maxSeconds] =
		burstCount > 1
			? [AI_AUTOREPLY_DEBOUNCE_BURST_MIN_SECONDS, AI_AUTOREPLY_DEBOUNCE_BURST_MAX_SECONDS]
...
async function persistInboundMediaToS3(params: {
	...
	try {
		...
		try {
			...
			const refreshed = await fetchMetaMediaMetadata(mediaId, params.accessToken)
...
function detectWhatsAppEventType(payload: any) {
	...
	const hasMessages = changes.some(
		(change: any) => Array.isArray(change?.value?.messages) && change.value.messages.length > 0,
	)
	const hasStatuses = changes.some(
		(change: any) => Array.isArray(change?.value?.statuses) && change.value.statuses.length > 0,
	)
...
function buildInstagramInboxIdCandidates(rawIds: Array<string | null | undefined>) {
...
async function findInstagramInbox(candidateIds: Array<string | null | undefined>) {
	const { uniqueIds, uniqueNumericIds } = buildInstagramInboxIdCandidates(candidateIds)
	if (uniqueIds.length === 0) return null

	const jsonIdMatchers: Array<Record<string, unknown>> = []
	for (const candidateId of uniqueIds) {
		jsonIdMatchers.push({ channel_config: { path: ['instagram_id'], equals: candidateId } })
		jsonIdMatchers.push({ channel_config: { path: ['fb_page_id'], equals: candidateId } })
	}
...
async function findTikTokInbox(candidateIds: Array<string | null | undefined>) {
	...
	for (const candidateId of uniqueIds) {
		jsonMatchers.push({ channel_config: { path: ['tiktok_id'], equals: candidateId } })
		jsonMatchers.push({ channel_config: { path: ['open_id'], equals: candidateId } })
		jsonMatchers.push({ channel_config: { path: ['account_id'], equals: candidateId } })
	}
...
function normalizeTikTokMediaType(value: unknown): 'image' | 'video' | 'audio' | 'document' | null {
	const normalized = String(value || '')
		.trim()
		.toLowerCase()
	if (!normalized) return null
...
function normalizeTikTokInboundEvents(payload: any): NormalizedTikTokInboundEvent[] {
	...
	for (const baseEvent of baseEvents) {
		...
		const contentText = text || (contentType === 'text' ? '' : `[${contentType}]`)
...
export function extractInstagramMessageContent(message: {
	...
	const replyTo = message.reply_to ? { reply_to: { mid: message.reply_to.mid } } : {}
...
async function fetchInstagramUserProfile(userId: string, accessToken: string, igBusinessAccountId?: string): Promise<{
	name: string | null
	...
	try {
		...
		console.warn(`[fetchInstagramUserProfile] Error fetching profile for ${userId}:`, error)
		return null
	}
}
...
async function persistInstagramMediaToS3(params: {
	...
	try {
		...
		console.error('[persistInstagramMediaToS3] Failed to persist Instagram media:', error)
		return null
	}
}
...
export abstract class WebhookService {
			...
			for (const entry of entries) {
				...
				for (const change of changes) {
					...
					if (!channel?.app_id || !channel?.inbox_id) {
						...
						errors.push(`Channel not found for phone_number_id=${phoneNumberId}`)
						continue
					}
	...
	static async handleTikTokInbound(payload: any, webhookEventId?: string) {
		...
		try {
			...
			if (events.length === 0) {
				...
				const eventName = asString(payloadRecord.event) || asString(payloadRecord.type)
				if (eventName && /message/i.test(eventName)) {
					...
					console.info('[WebhookService] Ignoring non-message TikTok webhook event', {
						event: eventName,
					})
				}
			}
			...
			for (const event of events) {
				...
				if (!inbox) {
					stats.unknownChannel += 1
					errors.push(`No inbox found for tiktok recipient_id=${event.recipientId}`)
	...
	static async processDebouncedAutoReplyJob(payload: any) {
		...
		if (!conversationId) {
			return { success: false, skipped: true, reason: 'missing_conversation_id' }
		}
		if (!params.appId || !params.inboxId || !params.contact || !params.channelType) {
			return { success: false, skipped: true, reason: 'invalid_payload' }
		}

		if (
			!(await this.isLatestAutoReplyToken(conversationId, debounceToken))
		) {
	...
	private static async scheduleChatbotAutoReply(params: AutoReplyParams) {
		...
		const debounceToken = `${Date.now().toString(36)}-${Math.random()
			.toString(36)
			.slice(2, 10)}`
		await redis.set(latestTokenKey, debounceToken, 'EX', AI_AUTOREPLY_TOKEN_TTL_SECONDS)
	...
	private static async isLatestAutoReplyToken(
		...
		const latestToken = await redis.get(buildAutoReplyLatestTokenKey(conversationId))
		return latestToken === debounceToken
	}

	private static async collectPendingInboundBatch(conversationId: string): Promise<{
		incomingText: string
	...
	private static async pickAssigneeByDistribution(params: {
		...
		const uniqueCandidateAgentIds = Array.from(
			new Set(params.candidateAgentIds.filter((agentId) => Boolean(asUuidOrNull(agentId)))),
		)
	...
	private static async escalateConversationToHuman(params: {
		...
		const uniqueConfiguredTeamIds = Array.from(
			new Set(params.defaultTeamIds.filter((teamId) => Boolean(asUuidOrNull(teamId)))),
		)
		const uniqueConfiguredAgentIds = Array.from(
			new Set(params.defaultAgentIds.filter((agentId) => Boolean(asUuidOrNull(agentId)))),
		)
		...
		const preferredDivisionName = String(params.preferredDivisionName || '').trim()
		...
		if (selectedAgentId) {
			await ConversationService.assignAgent(params.conversationId, selectedAgentId)
		}
	...
	private static async maybeSendChatbotAutoReply(params: AutoReplyParams) {
		...
		let preferredChatbotId =
			asUuidOrNull(inbox?.chatbot_id) || resolveConfiguredChatbotId(inboxChannelConfig)
		if (!preferredChatbotId) {
			preferredChatbotId = resolveConfiguredChatbotId(whatsappChannelMetadata)
		}

		const inboxHasTeamConfig =
			Object.prototype.hasOwnProperty.call(inboxChannelConfig, 'default_team_ids') ||
			Object.prototype.hasOwnProperty.call(inboxChannelConfig, 'defaultTeamIds')
		const inboxHasAgentConfig =
			Object.prototype.hasOwnProperty.call(inboxChannelConfig, 'default_agent_ids') ||
			Object.prototype.hasOwnProperty.call(inboxChannelConfig, 'defaultAgentIds')
		const inboxHasDistributionConfig =
			Object.prototype.hasOwnProperty.call(inboxChannelConfig, 'distribution_method') ||
			Object.prototype.hasOwnProperty.call(inboxChannelConfig, 'distributionMethod')
		...
		const escalationDistributionMethod = inboxHasDistributionConfig
			...
			: resolveConfiguredDistributionMethod(whatsappChannelMetadata) || 'round_robin'
		...
		if (
			...
			const appliedLabelName = syncedLabel?.title || response.meta.label_applied || null
			const appliedLabelId = syncedLabel?.id || response.meta.label_applied_id || null
			...
			const explicitHandoffStatusDetected = hasExplicitHandoffStatus(statusTimelineTexts)
			const aiHumanIdentityIntent = isAiVsHumanIdentityIntent(incomingText)
			const aiInternalQuestionIntent = isAiInternalOrModelQuestion(incomingText)
			const hardStopInternalIntent = aiHumanIdentityIntent || aiInternalQuestionIntent
				const inferredEscalationIntent =
					hardStopInternalIntent ||
					(Boolean(chatbot.agent_transfer) &&
						isLikelyEscalationIntent(incomingText, appliedLabelName))
			const shouldEscalateToHuman =
				explicitHandoffStatusDetected || inferredEscalationIntent
			const preferredDivisionName = extractPreferredDivisionName(statusTimelineTexts)
			if (shouldEscalateToHuman && !explicitHandoffStatusDetected) {
				const forcedReason = hardStopInternalIntent
					? 'Customer asking about AI/system internal.'
					: null
				const reasonText =
				...
				const existingStatusSet = new Set(
					statusTimelineTexts.map((text) => String(text || '').trim().toLowerCase()),
				)
				...
				...(segment.contentType === 'image' && segment.mediaUrl
					...
					...(isFirstSegment
						? {
								...
								ai_followups_matched: Number(response.meta.followups_matched || 0),
		...
		void AIResponseLogService.attachMessageIds({
			...
			messageIds: [
				...sentSystemMessages.map((item) => String(item?.id || '')).filter(Boolean),
		...
		const profile = await fetchInstagramUserProfile(senderId, inbox.config.page_access_token || inbox.config.access_token, inbox.config.instagram_id)
		...
		const contact = existingContact
			? await prisma.contacts.update({
					...
					data: {
						...
						instagram_igsid: senderId,
						name: existingContact.name === senderId || !existingContact.name ? contactName : existingContact.name,
						avatar_url: contactAvatar || existingContact.avatar_url || undefined,
						...
						updated_at: new Date(),
						meta: {
							...(typeof existingContact.meta === 'object' && existingContact.meta !== null ? existingContact.meta as any : {}),
							...contactMeta,
						},
					},
				})
		...
		if (contentAttributes.reply_to?.mid) {
			const replyToMsg = await prisma.messages.findFirst({
				where: { external_id: contentAttributes.reply_to.mid, deleted_at: null },
				select: { id: true },
			})
	...
	private static async emitInstagramMessageCreatedEvent(params: {
		...
		try {
			...
			console.error('[WebhookService] Failed to emit Instagram realtime event:', error)
		}
	}
	...
	private static async storeIncomingTikTokMessage(params: {
		...
		const messageExtras = buildMessageMediaExtras(event.contentType, contentAttributes)
	...
	private static async emitTikTokMessageCreatedEvent(params: {
		...
		try {
			...
			console.error('[WebhookService] Failed to emit TikTok realtime event:', error)
		}
	}
	...
	private static async storeIncomingWhatsAppMessage(params: {
		...
		const senderWaId = String(message?.from || value?.contacts?.[0]?.wa_id || '').trim()
		if (!senderWaId) {
			throw new Error('Incoming message missing sender wa_id')
		}

		const contactName =
			value?.contacts?.find((contact: any) => contact?.wa_id === senderWaId)?.profile?.name ||
			value?.contacts?.[0]?.profile?.name ||
			senderWaId

		const messageAt = parseUnixTimestamp(message?.timestamp)
		const windowExpiresAt = new Date(
			messageAt.getTime() + MESSAGING_WINDOW_HOURS * 60 * 60 * 1000,
		)
		...
		if (isMediaMessage) {
			const mediaPayload = (contentAttributes as any)?.media
			if (mediaPayload && typeof mediaPayload === 'object') {
				mediaUploadResult = await persistInboundMediaToS3({
				...
				if (mediaUploadResult?.publicUrl) {
					contentAttributes.media = {
						...
						s3_key: mediaUploadResult.key,
						mime_type:
							mediaPayload?.mime_type || mediaUploadResult.mimeType || undefined,
						sha256: mediaPayload?.sha256 || mediaUploadResult.sha256 || undefined,
						file_size:
							mediaPayload?.file_size || mediaUploadResult.fileSize || undefined,
						filename:
		...
		let replyToMessageId: string | null = null
		const waContextId = contentAttributes.context?.message_id
		if (waContextId) {
			const replyToMsg = await prisma.messages.findFirst({
				...
				if (mediaId) {
					const mediaOriginalUrl =
						typeof media?.original_url === 'string'
							? media.original_url
							: typeof message?.[parsedMessage.contentType]?.url === 'string'
								? message[parsedMessage.contentType].url
								: typeof media?.url === 'string'
									? media.url
									: null
					...
					await tx.media_files.create({
						data: {
							...
							download_status: mediaUploadResult?.publicUrl
								? 'downloaded'
								: 'pending',
							downloaded_at: mediaUploadResult?.publicUrl ? new Date() : null,
							url_expires_at: null,
							updated_at: new Date(),
						},
					})
	...
	static async getWhatsAppMediaContentByMessageId(messageId: string) {
		...
		if (uploadResult?.publicUrl) {
			...
			const nextExtras = buildMessageMediaExtras(contentType, nextContentAttributes)
		...
		try {
			...
			const refreshedMetadata = await fetchMetaMediaMetadata(mediaId, channel.api_key)
