...
const RAG_VECTOR_TOP_K = clampNumber(
	process.env.RAG_VECTOR_TOP_K,
	80,
	5,
	200,
)
...
const ENABLE_GLOBAL_KNOWLEDGE_COVERAGE =
...
function extractImageUrlContextsFromText(value: string): ImageUrlContext[] {
	...
	const pushCandidate = (urlRaw: string, start: number, end: number, hint = '') => {
	...
	while ((markdownMatch = markdownImageRegex.exec(source)) !== null) {
		...
		pushCandidate(url, markdownMatch.index, markdownImageRegex.lastIndex, altText)
	}
...
function scoreImageContextRelevance(context: string, keywords: Set<string>): number {
	if (!context) return 0
...
function extractKnowledgeImageMappings(
	...
	for (const item of knowledge) {
		...
		while ((markdownMatch = markdownImageRegex.exec(content)) !== null) {
			pushMapping(String(markdownMatch[1] || ''), String(markdownMatch[2] || ''))
		}
		...
		while ((lineMatch = lineRegex.exec(content)) !== null) {
			const label = String(lineMatch[1] || '').replace(/[#>*-]/g, '').trim()
...
function scoreGenericPosterPenalty(url: string): number {
	...
	if (
		normalized.includes('_general_') ||
		normalized.includes('tiktok_skin')
	) {
		penalty += 1
	}
	return penalty
...
function resolveMappedImageUrlByKeywords(args: {
	...
	const ranked = mappings
		...
		.sort((left, right) => {
			if (right.priceCue !== left.priceCue) return right.priceCue - left.priceCue
			if (right.score !== left.score) return right.score - left.score
			if (right.urlScore !== left.urlScore) return right.urlScore - left.urlScore
...
function extractImageCandidates(args: {
	...
	const registerCandidates = (
		...
		for (const contextItem of contexts) {
			...
			const priceCue = args.preferPriceImage
				...
				: 0
			const score = baseScore + relevance * 12 + urlKeywordScore * 16 + priceCue * 18
	...
	for (const toolRun of args.toolRuns) {
		...
		const textContexts = extractImageUrlContextsFromText(toolRun.responsePreview)
	...
	const ranked = Array.from(scoredByUrl.values())
		.sort((left, right) => {
			if (right.score !== left.score) return right.score - left.score
...
function resolveInlineImageUrlForSection(args: {
	...
	const effectiveKeywords = mergedKeywords.size > 0 ? mergedKeywords : baseKeywords
...
function normalizeInlineImageTargetsForResponse(args: {
	...
	const normalizedMessage = String(args.message || '').trim().toLowerCase()
	...
	const messageKeywords = tokenize(
		[args.message, inferredTreatment || '', stripInlineImageTokensFromText(source)]
			.filter(Boolean)
			.join(' '),
	)
	...
	for (const segment of segments) {
		...
		const sectionText = activeSectionText || stripInlineImageTokensFromText(source)
...
function buildImageSelectionKnowledgePool(args: {
	...
	const candidates = args.knowledgeSources
		.map<KnowledgeContextItem | null>((source) => {
			...
			const keywordScore = scoreByKeywordOverlap(args.messageKeywords, baseText)
...
function detectFirstMessageAdsExclusiveRule(args: {
	...
	const defaultTrigger = /halo\s*sozo.*tertarik\s*promo/i.test(normalizedMessage)
...
function classifyDynamicLabelKind(title: string): DynamicLabelKind {
	...
	if (normalized.includes('location') || normalized.includes('lokasi')) return 'location'
	if (normalized.includes('schedule') || normalized.includes('jadwal')) return 'schedule'
	if (
...
function resolveDynamicAppliedLabel(args: {
	...
	const isLocationIntent =
		/\b(lokasi|alamat|cabang|info lokasi|dimana|di mana|terdekat)\b/i.test(normalizedMessage) ||
	...
	const isPricePromoIntent =
		/\b(harga|price|pricelist|promo|diskon|deal|ppn|pajak)\b/i.test(normalizedMessage) &&
	...
	const scored = args.configuredLabels
		.map((label) => {
			...
			switch (kind) {
				case 'ads':
					score = /halo\s*sozo.*tertarik\s*promo/i.test(normalizedMessage) ? 200 : 0
					break
			...
			const titleScore = scoreByKeywordOverlap(tokenize(args.message), label.title) * 3
			return {
				id: label.id,
				title: label.title,
...
function capKnowledgeByTotalChars(
	...
	for (const item of items) {
		const boundedContent = truncateText(String(item.content || ''), boundedItem)
...
function buildKnowledgeCoverageContext(
	...
	for (let index = 0; index < items.length; index += 1) {
		...
		if (usedChars > 0 && usedChars + line.length + 2 > maxChars) {
			...
			if (remaining > 0) {
				lines.push(`... dan ${remaining} knowledge source lainnya tetap wajib dijadikan acuan.`)
			}
			break
		}
		lines.push(line)
...
function buildRagCacheKey(appId: string, chatbotId: string): string {
	return `${RAG_CACHE_PREFIX}${appId}:${chatbotId}:v1`
}
...
async function fetchKnowledgeCatalog(args: {
	...
	const cacheKey = buildRagCacheKey(args.appId, args.chatbotId)
	try {
		...
		if (cached) {
			...
			const cachedSources = Array.isArray(parsed.knowledgeSources)
				? parsed.knowledgeSources
						...
						.filter(
							(item): item is NonNullable<ReturnType<typeof normalizeKnowledgeSourceRecord>> =>
								item !== null,
						)
				: []
			const cachedFaqs = Array.isArray(parsed.faqs)
				? parsed.faqs
						...
						.filter(
							(item): item is NonNullable<ReturnType<typeof normalizeKnowledgeFaqRecord>> =>
								item !== null,
						)
				: []
	...
	const [knowledgeSources, faqs] = await Promise.all([
		...
		prisma.knowledge_faqs.findMany({
			...
			orderBy: [{ priority: 'desc' }, { updated_at: 'desc' }, { created_at: 'desc' }],
			take: 200,
			select: {
				id: true,
				question: true,
...
async function resolveEmbeddingVector(args: {
	...
	const embeddingModel = toNullableString(process.env.AI_EMBEDDING_MODEL) || 'text-embedding-3-small'
	const isAzure =
	...
	try {
		if (isAzure) {
			const deployment = args.runtime.deploymentName || embeddingModel
			const endpoint = joinUrl(
				args.runtime.baseUrl,
				`openai/deployments/${encodeURIComponent(deployment)}/embeddings?api-version=${encodeURIComponent(args.runtime.apiVersion || '2024-02-15-preview')}`,
			)
...
async function resolveVectorScores(args: {
	...
	try {
		const rows = (await prisma.$queryRawUnsafe(
			`
				...
				WHERE e.embedding IS NOT NULL
				  AND (
				  	(ks.id IS NOT NULL AND ks.app_id = $2::uuid AND ks.chatbot_id = $3::uuid AND COALESCE(ks.is_active, true) = true)
				  	OR
				  	(kf.id IS NOT NULL AND kf.app_id = $2::uuid AND kf.chatbot_id = $3::uuid AND COALESCE(kf.is_active, true) = true)
				  )
		...
		for (const row of rows) {
			...
			const key = sourceId ? `source:${sourceId}` : faqId ? `faq:${faqId}` : null
			if (!key) continue
...
function resolveRetrievalIntent(args: {
	...
	if (!args.strictFollowupMode && inferLocationIntent({ message: normalizedMessage })) {
		return 'location'
	}
	if (
		isPriceConfirmationIntent(normalizedMessage) ||
...
function prioritizeKnowledgeForAccuracy(args: {
	...
	if (strong.length > 0) return strong.sort((left, right) => right.score - left.score)
	if (fallback.length > 0) return fallback.sort((left, right) => right.score - left.score)
	return workingSet
}
...
function compressKnowledgeWithRTK(args: {
	...
	for (const item of args.items) {
		...
		const stableId =
			toNullableString(item.id) || `${item.type}:${normalizeToolLookupKey(item.title)}`
...
function serializeKnowledgeReferences(items: KnowledgeContextItem[]): KnowledgeReferenceLog[] {
...
function buildActiveToolsContext(args: {
	...
	const configuredNames = Array.from(
		new Set(args.activeConfiguredToolNames.map((name) => String(name || '').trim()).filter(Boolean)),
	)
	const candidateByName = new Map(
		args.candidateTools.map((tool) => [normalizeToolLookupKey(tool.name), tool] as const),
	)
	const availableByName = new Map(
		args.availableTools.map((tool) => [normalizeToolLookupKey(tool.name), tool] as const),
	)
	...
	if (prioritizedTools.length === 0 && configuredNames.length === 0) return null

	const lines: string[] = []
	if (prioritizedTools.length > 0) {
		for (const tool of prioritizedTools.slice(0, 20)) {
			const required = Array.isArray(tool.required) && tool.required.length > 0
				? tool.required.join(', ')
				: '-'
...
function buildEvaluationContext(
	...
	if (evaluations.length === 0) return 'Tidak ada evaluation aktif yang tersimpan.'
	...
	if (evaluations.length > 50) {
		lines.push(`- ... ${evaluations.length - 50} evaluation lainnya juga tersedia.`)
	}
...
function normalizeHistory(history: unknown, maxItems: number): NormalizedHistoryMessage[] {
...
function isPriceConfirmationIntent(message: string): boolean {
	...
	const hasNominal = /\brp\.?\s*\d[\d.,]*|\b\d{2,5}\s*(rb|ribu|k)\b/.test(normalized)

	return (hasPriceKeyword && (hasConfirmKeyword || hasNominal)) || (hasConfirmKeyword && hasNominal)
}
...
function inferRecentTreatmentFromHistory(history: NormalizedHistoryMessage[]): string | null {
	const patterns = [
	...
	for (const item of [...history].reverse()) {
		...
		for (const pattern of patterns) {
			...
			const normalizedCandidate = normalizeTreatmentContextCandidate(rawCandidate)
...
function parseTreatmentHeadingLine(value: string): { name: string; imageUrl: string | null } | null {
...
function extractNormalizedPriceMentions(value: string): string[] {
	...
	const priceRegexes = [/(rp\.?\s*[0-9][0-9.,]*)/gi, /([0-9][0-9.,]*\s*(?:rb|ribu|k))/gi]
...
function buildStrictTreatmentPriceResponse(args: {
	...
	if (entry.normalMember) details.push(`- Harga normal member: ${entry.normalMember}`)
	if (entry.promo) {
		details.push(
...
function applyStrictTreatmentPriceFormatter(args: {
	...
	const hasAllowedPrice = mentionedPrices.some((price) => allowedPrices.has(price))
	...
	const hasTreatmentMismatch = Boolean(
		responseTreatment && !isTreatmentNameAligned(responseTreatment, matchedEntry.name),
	)

	const inlineImageUrls = splitInlineContentSegments(currentContent)
		.filter((segment): segment is { type: 'image'; url: string } => segment.type === 'image')
		.map((segment) => normalizeHttpUrl(segment.url))
		.filter((url): url is string => Boolean(url))
	const hasImage = inlineImageUrls.length > 0
	const normalizedExpectedImageUrl = normalizeHttpUrl(matchedEntry.imageUrl || '')
	const hasDisallowedImage = Boolean(
		normalizedExpectedImageUrl &&
			inlineImageUrls.some((url) => url !== normalizedExpectedImageUrl),
	)

	if (hasTreatmentMismatch || hasDisallowedPrice || !hasAllowedPrice || hasDisallowedImage) {
...
function extractTreatmentPriceContext(args: {
	...
	const best = catalog
		.map((entry) => {
			...
			score += scoreByKeywordOverlap(treatmentTokens, entry.normalizedName) * 10
			score += Math.floor(entry.score / 8)
			return {
				entry,
				score,
...
function shouldApplyDeterministicPriceFallback(args: {
	...
	const hasPriceNominal = /\brp\.?\s*\d[\d.,]*|\b\d{2,5}\s*(rb|ribu|k)\b/i.test(current)
...
function extractLocationMentionFromText(message: string): string | null {
	...
	if (/^[a-z0-9 .,'-]{3,48}$/i.test(normalized)) {
		...
		if (containsAnyKeyword(normalized, LOCATION_REPLY_EXCLUSION_KEYWORDS)) return null
		const candidate = normalizeLocationCandidate(normalized)
		if (candidate && !containsAnyKeyword(candidate.toLowerCase(), LOCATION_INTENT_KEYWORDS)) {
			return candidate
		}
	}
...
function buildToolLookupContext(tool: APIToolRecord): string {
	...
	const propertyTokens = toRecordArray(tool.properties).flatMap((property) => {
		...
		const enumValues = Array.isArray(property.enum)
			...
			: []
		return [name, description, ...enumValues].filter((item): item is string => Boolean(item))
	})
...
function isLocationIntentMessage(message: string): boolean {
	...
	if (locationPhraseMatch) {
		const locationCandidate = normalizeLocationCandidate(locationPhraseMatch[1] || '')
...
function scoreToolRelevance(args: {
	...
	const keywordScore = scoreByKeywordOverlap(args.messageKeywords, lookupContext)
	...
	const firstTurnBoost =
		args.history.length === 0 && /greet|welcome|halo/i.test(args.tool.name) ? 1 : 0
...
function hasRecurringPackageOffer(content: string): boolean {
	return /\bpaket\b|\bberulang\b|\bcombo\b|\bbundle\b/i.test(String(content || ''))
}

function appendOfferBeforeLocationPrompt(content: string, offer: string): string {
	const normalizedContent = String(content || '').trim()
	const normalizedOffer = String(offer || '').trim()
	if (!normalizedContent || !normalizedOffer) return normalizedContent || normalizedOffer
...
function normalizeToolMethod(value: unknown): 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE' {
	const normalized = String(value || '')
		.trim()
		.toUpperCase()
	if (
...
async function executeApiTool(args: {
	...
	const requiredKeys = Array.isArray(args.tool.required)
		? args.tool.required.map((item) => String(item || '').trim()).filter(Boolean)
		: []
...
async function resolveProviderRuntime(args: {
	...
	const temperature = clampNumber(
		args.chatbot.temperature ?? runtimeProvider?.temperature ?? settings?.temperature,
		0.2,
		0,
		2,
	)
...
async function requestModelResponse(args: {
	...
	const estimatedUsage = estimateUsageFromMessages({
		messages: llmMessages.map((item) => ({ role: item.role, content: item.content })),
		completionText: '',
	})
...
function buildSafeKnowledgeSnippetForFallback(args: {
	...
	const sanitized = sanitizeAssistantResponseForDelivery(extracted || plainContent)
...
function buildFallbackResponse(args: {
	...
	const greeting = args.chatbot.welcome_msg || `Halo, saya ${args.chatbot.name}.`
	const successfulTool = args.toolRuns.find((item) => item.ok && item.responsePreview)
	...
	const topKnowledge = args.knowledge.find((item) => item.score > 0) || args.knowledge[0]
	if (topKnowledge) {
		const snippet = buildSafeKnowledgeSnippetForFallback({
			message: args.message,
		...
		if (snippet) {
			return [
				greeting,
				snippet,
				'Kalau Kakak mau, aku bisa bantu arahin langkah berikutnya sesuai kebutuhan Kakak.',
			].join('\n\n')
		}
	}
...
function cleanLocationTextValue(value: string | null | undefined): string | null {
...
function parseLocationBranchRowsFromUnknown(
	...
	if (Array.isArray(value)) {
		return value.flatMap((item) => parseLocationBranchRowsFromUnknown(item, depth + 1))
	}
	...
	const hasLocationShape =
		...
		(Boolean(name && /clinic|sozo|branch|cabang/i.test(name)) ||
			Boolean(address && /jalan|jl\.|kec\.|kel\.|kab\.|kota|kecamatan|kabupaten/i.test(address)) ||
...
function parseLocationBranchRowsFromText(value: string): LocationBranchRow[] {
	...
	for (const line of lines) {
		...
		const addressMatch = normalizedLine.match(/^(?:alamat|address)\s*[:\-]\s*(.+)$/i)
		if (addressMatch) {
		...
		if (/^(?:maps?|google maps?)\s*[:\-]/i.test(normalizedLine)) {
			current.mapsUrl = toFirstUrl(normalizedLine) || cleanLocationTextValue(normalizedLine)
			continue
		}
...
function dedupeLocationBranchRows(rows: LocationBranchRow[]): LocationBranchRow[] {
	const seen = new Set<string>()
...
function buildLocationResponseFromKnowledge(args: {
	...
	const filteredRows = (hasMatch
		? prioritized.filter((item) => item.score > 0)
		: prioritized
	).map((item) => item.row)
...
function parseLocationRowsFromToolPreview(responsePreview: string): LocationBranchRow[] {
	const rows: LocationBranchRow[] = []
...
function buildExecutedToolStatusDetails(args: {
	...
	for (const run of args.toolRuns) {
		...
		if (isLocationToolName(toolName)) {
			const locationRows = parseLocationRowsFromToolPreview(run.responsePreview)
...
function extractPersonaGreetingFromPrompt(prompt: string | null | undefined): string | null {
	...
	const roleOrgMatch = normalized.match(/^\s*role\s*:\s*[^\n\r]*?[–-]\s*([^\n\r]+)/im)
...
function extractSequenceTwoVoucherClosingFromPrompt(
	...
	const directMatch =
		scopedMatch?.[1] ||
		source.match(/(Oh iya kak[\s\S]{0,700}?weekend atau weekdays[\s\S]{0,120}?\?)/i)?.[1] ||
		voucherMatch?.[1]
...
function resolveSequenceTwoVoucherClosing(args: {
	...
	const fromAllRules = extractSequenceTwoVoucherClosingFromRules(args.allFollowups)
	if (fromAllRules) {
		return fromAllRules
	}
...
function selectPromoContextSnippet(
	...
	const isMetaPromptText = (value: string): boolean =>
		/(core identity|tone of voice|objectives|communication style|discovery questions|safety|guard rail|instruksi agent|panduan handoff|role: ai)/i.test(
			value,
		)
	const promoParagraphFrom = (value: string): string | null => {
		...
		const preferred = candidates.find((item) => {
			if (isMetaPromptText(item)) return false
		...
		if (isMetaPromptText(normalized)) return null
		return truncateText(normalized, 450)
	}

	const successfulToolPreview = toolRuns
		.filter((item) => item.ok && item.responsePreview)
		.map((item) => decodeAssistantPayloadText(String(item.responsePreview || '').trim()))
		.map((item) => promoParagraphFrom(item))
		.find((preview) => {
			if (!preview) return false
			return isPromoInfoIntent(preview) || /harga|promo|cabang|alamat|maps/i.test(preview)
		})
	...
	const bestKnowledge = knowledge.find((item) => {
		...
		if (isMetaPromptText(haystack)) return false
		return isPromoInfoIntent(haystack) || /harga|promo|cabang|alamat|maps/.test(haystack)
	})
	if (bestKnowledge) {
...
function buildPromoFirstResponse(args: {
	...
	const snippet =
		messageLower.includes('intimate glow')
...
function hasStructuredServiceCatalog(content: string): boolean {
	...
	const hasCategories = /(perawatan wajah|perawatan tubuh|perawatan rambut)/i.test(
		normalized,
	)
...
function buildServiceCatalogCategories(
	...
	const corpus = knowledgeSources
		.map((source) => stripHtml(`${source.title || ''}\n${source.content || ''}`))
		.join('\n')
		.toLowerCase()
...
function buildServiceCatalogResponse(args: {
	...
	const itemDescriptions: Record<string, string> = {
		'Glass Skin Facial': 'Facial untuk membantu kulit tampak lebih bersih dan glowing.',
		'SOZO Signature Facial': 'Facial signature untuk membersihkan sekaligus menyegarkan wajah.',
		'Acne Laser Facial': 'Perawatan laser untuk jerawat aktif dan bekas jerawat.',
		'IPL Acne': 'Perawatan cahaya untuk membantu mengurangi jerawat dan kemerahan.',
		'Meso Acne': 'Mesoterapi untuk kulit berjerawat dan produksi minyak berlebih.',
		'Skin Booster': 'Booster untuk hidrasi kulit dan membantu kulit tampak lebih kenyal.',
		HIFU: 'Treatment non-invasif untuk membantu mengencangkan area wajah.',
		'Body Spot Repair': 'Perawatan tubuh untuk membantu area kulit belang/gelap.',
		'Body Whitening Peel': 'Peeling tubuh untuk membantu mencerahkan tampilan kulit.',
		'Slimming Treatment': 'Program body contouring/slimming sesuai kebutuhan.',
		'PRP Hair': 'Perawatan untuk membantu mengurangi rambut rontok dan menstimulasi pertumbuhan rambut.',
		'Hair Grow': 'Treatment stimulasi pertumbuhan rambut dengan evaluasi dokter.',
		'Biolight Hair': 'Perawatan cahaya untuk kesehatan kulit kepala dan rambut.',
	}
	const genericCategoryDescription: Record<string, string> = {
		'Perawatan Wajah': 'Perawatan wajah sesuai concern kulit.',
		'Perawatan Tubuh': 'Perawatan tubuh untuk pencerahan dan perawatan area spesifik.',
		'Perawatan Rambut': 'Perawatan rambut dan kulit kepala.',
	}
	const sections = categories.map((category) => {
		const lines = category.items.slice(0, 6).map((item) => {
			const description =
				itemDescriptions[item] || genericCategoryDescription[category.title] || 'Perawatan sesuai kebutuhan.'
...
export abstract class ChatbotSimulationService {
	static async simulateResponse(
		...
		const [{ knowledgeSources, faqs }, evaluations] = await Promise.all([
		...
		const embeddingVector = await resolveEmbeddingVector({
			runtime,
			message: retrievalMessage,
		})
		...
		const rankedSources: KnowledgeContextItem[] = knowledgeSources.map((source) => {
			const content = stripHtml(String(source.content || ''))
			const baseText = `${source.title || ''}\n${content}`.trim()
			const keywordScore = scoreByKeywordOverlap(messageKeywords, baseText)
			const vectorScore = vectorScoreByKnowledge.get(`source:${source.id}`) || 0
			return {
				type: 'source',
				id: source.id,
				title: source.title || source.type || 'Knowledge Source',
		...
		const relevanceFilteredKnowledge = filterKnowledgeByRelevance(rankedKnowledge, {
		...
		let selectedKnowledge = compressedKnowledge.items
			...
			const shouldRunTools = input.runTools !== false && !strictFollowupMode
			if (shouldRunTools) {
			...
			if (!appliedLabelName && configuredLabelIds.length === 0) {
				appliedLabelName = extractLabelFromCondition(input.chatbot.label_condition)
				if (appliedLabelName) {
			...
			systemPromptSections.push('Gunakan bahasa yang sama dengan bahasa user, default Bahasa Indonesia.')
		...
		const appData = toRecord(input.chatbot.app_data)
			...
			const knowledgeCoverageContext =
				ENABLE_GLOBAL_KNOWLEDGE_COVERAGE
					? buildKnowledgeCoverageContext(rankedKnowledge, 8_000)
					: null
			if (knowledgeCoverageContext && selectedKnowledge.length === 0) {
				systemPromptSections.push(
			...
			const successfulTools = toolRuns.filter((item) => item.ok && item.responsePreview)
		...
		let resolvedContent =
			(modelResponse.content && modelResponse.content.trim()) || fallbackResponse

			const sanitizedResolvedContent = sanitizeAssistantResponseForDelivery(
				resolvedContent,
			)
			if (sanitizedResolvedContent) {
				resolvedContent = sanitizedResolvedContent
		} else {
			const sanitizedFallback = sanitizeAssistantResponseForDelivery(
				fallbackResponse,
			)
			resolvedContent =
				sanitizedFallback ||
		...
		const resolvedModelName = runtime.modelName || input.chatbot.model || 'standard'
		...
		const hasInlineImages = inlineSegments.some((segment) => segment.type === 'image')
