	...
	const staleBefore = new Date(
		...
		WHERE id = ${params.conversationId}
			...
			AND (
				...
				OR CASE
					WHEN COALESCE(additional_attributes->${CHATBOT_FOLLOWUP_STATE_KEY}->>'processing_started_at', '') ~ '^\\d{4}-\\d{2}-\\d{2}T'
	...
	static async dispatchDueFollowupsBatch(limit = 100) {
		...
		const dueRows = await prisma.$queryRaw<Array<{ id: string }>>`
			...
			WHERE deleted_at IS NULL
				...
				AND (additional_attributes->${CHATBOT_FOLLOWUP_STATE_KEY}->>'next_due_at') ~ '^\\d{4}-\\d{2}-\\d{2}T'
