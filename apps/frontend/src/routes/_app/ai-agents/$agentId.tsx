...
import { Pencil, LayoutGrid, MessageCircle, Check, Copy } from 'lucide-react'
...
const AI_TOOLS_AGENT_ID = '1c444e9f-f06c-4fe1-b1b7-7ddb35a6f1b9'
...
function AIAgentDetailPage() {
	...
	useEffect(() => {
		...
		} else if (agentId === AI_TOOLS_AGENT_ID) {
	...
	// Knowledge Base states (Documents API structure)
	...
	const [knowledgeTab, setKnowledgeTab] = useState<
		'text' | 'website' | 'file' | 'qna' | 'product'
	>('text')
	const [showAddSourceModal, setShowAddSourceModal] = useState(false)
	const [newSourceData, setNewSourceData] = useState({
		title: '',
		content: '',
		type: 'text' as 'text' | 'website' | 'evaluation' | 'prompt' | 'qna',
	})
	const [addingSource, setAddingSource] = useState(false)

	// Text documents state
	const [textDocuments, setTextDocuments] = useState<any[]>([])
	const [activeTextDoc, setActiveTextDoc] = useState<string | null>(null)
	const [editingTextDoc, setEditingTextDoc] = useState<any>(null)
	...
	const [linkMode, setLinkMode] = useState<'batch' | 'single'>('batch')
	const [newLinkUrl, setNewLinkUrl] = useState('')
	const [collectingLink, setCollectingLink] = useState(false)
	...
	useEffect(() => {
		if (knowledgeTab === 'text' && textDocuments.length > 0 && !activeTextDoc) {
			setActiveTextDoc(textDocuments[0].id)
			setEditingTextDoc({ ...textDocuments[0] })
		}
	}, [knowledgeTab, textDocuments])
	...
	const fetchKnowledgeData = async () => {
		try {
			// Fetch documents from the external API structure
			...
			if (docsData.data || docsData.payload) {
				...
				// Helper function to parse evaluation content
				...
				documents.forEach((doc: any) => {
					const charCount = (doc.content || doc.html || '').length
					...
					switch (doc.type) {
						case 'text':
							...
							break
						case 'prompt':
							// Prompts are also shown as text documents but with different handling
							texts.push({
								id: doc.id,
								title: doc.title || 'System Prompt',
								content: doc.content || '',
								type: doc.type,
								created_at: doc.created_at,
							})
							break
						case 'evaluation': {
							// Parse the embedded question/answer from content
						...
						case 'pdf':
							files.push({
								id: doc.id,
								name: doc.title || doc.url || 'Unnamed File',
								type: doc.type,
								created_at: doc.created_at,
							})
							break
						case 'product':
							...
							break
						default:
							// Treat as text if type is unknown
							texts.push({
								id: doc.id,
								title: doc.title || 'Untitled',
								content: doc.content || doc.html || '',
				...
				setTextDocuments(texts)
				setQnaItems(qnas)
				setWebsiteLinks(links)
				setUploadedFiles(files)
				setProductItems(products)

				if (texts.length > 0 && !activeTextDoc && knowledgeTab === 'text') {
					setActiveTextDoc(texts[0].id)
					setEditingTextDoc({ ...texts[0] })
				}

				// Calculate total text characters from all text documents
				const textChars = texts.reduce(
					(acc, t) => acc + (t.content?.length || 0),
					0,
				)

				setKnowledgeStats({
					sources_count: documents.length,
					faqs_count: qnas.length,
					categories_count: 0,
					files_count: files.length,
					text_characters: textChars,
					links_count: links.length,
					qna_count: qnas.length,
					total_characters: totalChars,
				})

				// If there are text documents, select the first one
				if (texts.length > 0 && !activeTextDoc) {
					setActiveTextDoc(texts[0].id)
					setEditingTextDoc(texts[0])
				}
			}

			// Also try to fetch legacy knowledge data for backwards compatibility
			try {
				const [sourcesRes, faqsRes, catsRes] = await Promise.all([
					fetch(`${API_URL}/api/knowledge?chatbot_id=${agentId}`, {
						headers: { Authorization: `Bearer ${token}` },
					}),
					fetch(`${API_URL}/api/knowledge/faqs?chatbot_id=${agentId}`, {
						headers: { Authorization: `Bearer ${token}` },
					}),
					fetch(`${API_URL}/api/knowledge/categories?chatbot_id=${agentId}`, {
						headers: { Authorization: `Bearer ${token}` },
					}),
				])

				const [sourcesData, faqsData, catsData] = await Promise.all([
					sourcesRes.json(),
					faqsRes.json(),
					catsRes.json(),
				])

				if (sourcesData.data) setKnowledgeSources(sourcesData.data)
				if (faqsData.payload) setFaqs(faqsData.payload)
				if (catsData.payload) setCategories(catsData.payload)
			} catch (legacyError) {
				// Legacy endpoints might not exist, that's okay
				console.log('Legacy knowledge endpoints not available')
			}
		} catch (error) {
	...
	const handleAddSource = async () => {
		if (!newSourceData.title || !newSourceData.content) {
			toast.error('Please fill in both title and content/URL')
			return
		}
		...
		try {
			...
			if (res.ok) {
				...
				toast.error(error.error || 'Failed to add knowledge source')
			}
		} catch (error) {
	...
	const handleDeleteSource = async (id: string) => {
		...
		try {
			...
			if (res.ok) {
				...
				setTextDocuments((prev) => prev.filter((d) => d.id !== id))
				setQnaItems((prev) => prev.filter((d) => d.id !== id))
				setWebsiteLinks((prev) => prev.filter((d) => d.id !== id))
				setUploadedFiles((prev) => prev.filter((d) => d.id !== id))
				setProductItems((prev) => prev.filter((d) => d.id !== id))
				// Clear editing state if deleted document is being edited
				if (activeTextDoc === id) {
					setActiveTextDoc(null)
	...
	const handleDeleteCategory = async (id: string) => {
		...
		try {
			const res = await fetch(`${API_URL}/api/knowledge/categories/${id}`, {
				method: 'DELETE',
				headers: { Authorization: `Bearer ${token}` },
			})
	...
	const handleSaveTextDocument = async () => {
		if (!editingTextDoc) return false

		setSaving(true)
		try {
			// Check if this is a new document (created locally but not yet saved to server)
			const isNewDoc = editingTextDoc.id.toString().length < 20 // Local IDs are timestamps
			...
			if (isNewDoc) {
				...
				if (res.ok) {
					const data = await res.json()
					// Replace the temporary local document with the server version
					setTextDocuments((prev: any[]) =>
						prev.map((d) => (d.id === editingTextDoc.id ? data.data : d)),
					)
					setActiveTextDoc(data.data.id)
					setEditingTextDoc(data.data)
				...
				const res = await fetch(
					`${API_URL}/api/chatbots/${agentId}/documents/${editingTextDoc.id}`,
					{
						method: 'PATCH',
						headers: {
							Authorization: `Bearer ${token}`,
				...
				if (res.ok) {
					setTextDocuments((prev: any[]) =>
						prev.map((d) => (d.id === editingTextDoc.id ? editingTextDoc : d)),
					)
					setHasUnsavedDocChanges(false)
	...
	const handleSave = async () => {
		...
		try {
			...
			// 2. Save Knowledge Sources (Batch if any)
			if (activeTab === 'knowledge') {
				// Batch Add Selected Links
				if (knowledgeTab === 'website' && selectedLinks.length > 0) {
				...
				// Batch Add Files (Requires FormData)
				if (knowledgeTab === 'file' && filesToAdd.length > 0) {
					// Note: Current api/knowledge might not handle PDF yet properly in this format
					// But we can implement a separate upload endpoint or loop
					for (const file of filesToAdd) {
						const formDataFile = new FormData()
						formDataFile.append('file', file)
						formDataFile.append('chatbot_id', agentId!)

						await fetch(`${API_URL}/api/media/upload-knowledge`, {
							method: 'POST',
							headers: { Authorization: `Bearer ${token}` },
							body: formDataFile,
						})
	...
	const tabs = [
		...
		{ id: 'advanced', label: 'Advanced', icon: FileStack },
	]
		...
		<div className="flex-1 flex flex-col h-full bg-gray-50 overflow-hidden">
				...
				<div className="flex items-center justify-center gap-1 overflow-x-auto">
					{tabs.map((tab) => (
						<button
							key={tab.id}
							onClick={() => {
								const switchTab = () => setActiveTab(tab.id as Tab)
								if (hasUnsavedDocChanges) {
									setPendingAction(() => switchTab)
			...
			<div className="flex-1 overflow-y-auto">
						...
						<div className="flex-1 space-y-6">
							{activeTab === 'general' && (
								<>
									...
									{/* Welcome Message */}
									<div className="bg-white rounded-xl border border-gray-200 p-6">
										<h3 className="text-blue-600 font-semibold text-center mb-1 flex items-center justify-center gap-1">
											Welcome Message
										...
										<a
											href="#"
											className="text-blue-500 text-xs block text-center mb-4 hover:underline"
										>
										...
										<div className="relative">
											<textarea
												...
												onChange={(e) =>
													setFormData({
														...formData,
														welcome_msg: e.target.value,
													})
												}
												rows={3}
												className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none"
												placeholder="Message to new customers..."
											/>
											<div className="absolute bottom-3 right-3 text-xs text-gray-400">
												{formData.welcome_msg.length}/5000
											</div>
										</div>
									</div>
											...
											<div className="space-y-6">
												<div className="flex items-start justify-between gap-4">
													...
													<button
														...
														className={`relative inline-flex h-11 w-20 shrink-0 items-center rounded-full transition-colors ${
															formData.stop_after_handoff
																? 'bg-blue-600'
																: 'bg-gray-300'
														}`}
							...
							{activeTab === 'knowledge' && (
								<div className="flex gap-6">
									...
									<div className="flex-1 space-y-4">
										...
										<div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
											<div className="flex items-center gap-1 p-2 border-b border-gray-100">
												{[
													...
													{ id: 'file', label: 'File', icon: FileStack },
													...
													<button
														...
														onClick={() => {
															const switchKTab = () =>
																setKnowledgeTab(t.id as any)
															if (hasUnsavedDocChanges) {
																setPendingAction(() => switchKTab)
																setShowUnsavedModal(true)
															} else {
																switchKTab()
															}
										...
										{/* Text Tab Content */}
										{knowledgeTab === 'text' && (
											<div className="flex flex-col h-full bg-white rounded-lg border border-gray-200 mt-4 overflow-hidden">
												{/* Header: Tabs + Actions */}
												<div className="flex items-center border-b border-gray-200 bg-white">
													...
													<button
														onClick={() => {
															...
															setActiveTextDoc(newId)
															setEditingTextDoc(newDoc)
															setRenamingDoc(newId)
															setHasUnsavedDocChanges(true)
														}}
														className="p-3 border-r border-gray-200 hover:bg-gray-50 transition-colors text-gray-500"
													>
														<Plus size={20} />
													</button>
													...
													<div
														ref={tabsRef}
														className="flex flex-1 overflow-x-auto scrollbar-hide ml-1 mr-4"
													>
														{textDocuments.length === 0 && (
															<div className="flex items-center h-10 px-4 text-sm text-gray-400 italic">
																No documents. Click + to add.
															</div>
														)}
														{textDocuments.map((doc: any) => {
															const isActive = activeTextDoc === doc.id
															return (
																<div
																	key={doc.id}
																	className={`
                                                                            relative flex items-center min-w-[140px] max-w-[240px] h-10
                                                                            px-4 pr-16
                                                                            transition-all duration-200 select-none cursor-pointer
                                                                            ${
																																							isActive
																																								? 'bg-blue-500 text-white z-10 shadow-md rounded-t-xl border-t border-l border-r border-blue-400'
																																								: 'bg-gray-50/80 text-gray-700 hover:bg-gray-100 border-r border-gray-200'
																																						}
                                                                        `}
																	onClick={() => {
																		const switchDoc = () => {
																	...
																	<div className="flex items-center w-full gap-1">
																		{renamingDoc === doc.id ? (
																			...
																			>
																				<input
																					autoFocus
																					className="bg-transparent text-sm w-full outline-none border-b border-white/50"
																					value={editingTextDoc?.title || ''}
																					onChange={(e) => {
																			...
																			<span className="truncate text-sm flex-grow font-medium">
																				{doc.title || 'Untitled'}
																			</span>
																		)}
																	</div>
																	{/* Edit & Close Buttons - Always visible */}
																	{renamingDoc !== doc.id && (
																		<div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-0.5">
																			<button
																				...
																				className={`p-1 rounded transition ${isActive ? 'hover:bg-blue-400 text-white/80' : 'hover:bg-gray-200 text-gray-400'}`}
																			>
																				<Pencil size={12} />
																			</button>
																			<button
																				...
																				className={`p-1 rounded transition ${isActive ? 'hover:bg-blue-400 text-white/80' : 'hover:bg-gray-200 text-gray-400 hover:text-red-500'}`}
																			>
																				<X size={12} />
																			</button>
																		</div>
													...
													{/* Actions: Nav + Save */}
													<div className="flex items-center gap-1 pr-4 border-l border-gray-100 pl-2 h-10">
														<button
															onClick={() =>
															...
															className="p-1.5 hover:bg-gray-50 text-gray-400 rounded transition flex items-center justify-center"
														>
															<ChevronLeft size={18} />
														</button>
														<button
														...
														<button
															...
															disabled={!hasUnsavedDocChanges || saving}
															className={`ml-2 px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-1.5 transition ${
										...
										{knowledgeTab === 'website' && (
											<div className="p-6 min-h-[500px]">
												<div className="mb-6">
													...
													<p className="text-sm text-gray-500">
														Provide a link to the page you want the AI to learn
														from.
													</p>
												</div>
												...
												<div className="mb-6">
													...
													<div className="flex gap-3">
														...
														<button
															onClick={async () => {
																...
																setCollectingLink(true)
																try {
																	const response = await fetch(
																		`${import.meta.env.VITE_API_URL || '/api'}/knowledge/collect-links`,
																		{
																			method: 'POST',
																			headers: {
																				'Content-Type': 'application/json',
																	...
																	if (result.success) {
																		setWebsiteLinks(result.payload)
																		toast.success(
																			'${result.payload.length} links collected successfully',
																		)
																		setNewLinkUrl('')
																	} else {
																	...
																	toast.error('Failed to collect links')
																} finally {
																	setCollectingLink(false)
																}
															}}
															disabled={collectingLink || !newLinkUrl}
													...
													<p className="text-xs text-blue-600 mt-2 flex items-center gap-1">
														<Info size={12} />
														Start with URL and this tool will gather up to{' '}
														<strong>300 unique</strong> links from the site,
														excluding any files.
													</p>
												</div>
													...
													<div className="flex items-center gap-3 mb-4">
														<input
															...
															checked={
																...
																selectedLinks.length === websiteLinks.length
															}
															onChange={(e) => {
																if (e.target.checked) {
																	setSelectedLinks(
																		websiteLinks.map((l) => l.url),
																	)
																} else {
																	setSelectedLinks([])
																}
										...
										{knowledgeTab === 'file' && (
											<div className="p-6 min-h-[500px]">
												...
												<div className="mt-6">
													<h4 className="font-medium text-gray-700 mb-3">
														To be added: {filesToAdd.length > 0 ? '' : '-'}
													</h4>
													{filesToAdd.length > 0 && (
														<div className="space-y-2">
						...
						<div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
							...
							<div className="p-6 space-y-4">
								<div>
									...
									<div className="grid grid-cols-2 gap-3">
										<button
											onClick={() =>
												setNewSourceData({ ...newSourceData, type: 'text' })
											}
											className={`flex items-center justify-center gap-2 p-3 rounded-xl border transition ${newSourceData.type === 'text' ? 'bg-blue-50 border-blue-500 text-blue-600' : 'bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100'}`}
										>
											<FileText size={18} />
											<span className="font-medium text-sm">Text Snippet</span>
										</button>
										<button
											onClick={() =>
												setNewSourceData({ ...newSourceData, type: 'website' })
											}
											className={`flex items-center justify-center gap-2 p-3 rounded-xl border transition ${newSourceData.type === 'website' ? 'bg-blue-50 border-blue-500 text-blue-600' : 'bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100'}`}
										>
											<Globe size={18} />
								...
								<div>
									<label className="block text-sm font-medium text-gray-700 mb-1">
										{newSourceData.type === 'text' ? 'Content' : 'URL'}
									</label>
									<textarea
										rows={8}
										placeholder={
									...
									/>
									{newSourceData.type === 'website' && (
										<p className="text-[10px] text-gray-400 mt-1 italic">
											We will crawl the page and extract readable text
											automatically.
										</p>
									)}
								</div>
							...
							<div className="px-6 py-4 bg-gray-50 flex items-center justify-end gap-3">
								...
								<button
									...
									className="px-6 py-2 bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-600 transition shadow-sm disabled:opacity-50 flex items-center gap-2 text-sm"
								>
									{addingSource ? (
										<>
											<RefreshCw className="animate-spin" size={16} />
								...
								<button
									onClick={async () => {
										...
										try {
											...
											setTextDocuments((prev) => [...prev, newDoc])
											setActiveTextDoc(newDoc.id)
											setEditingTextDoc(newDoc)
											setShowAddTextModal(false)
											setNewSourceData({
												title: '',
											...
											toast.error('Failed to add text document')
										} finally {
											setAddingSource(false)
										}
									}}
									disabled={addingSource}
									className="px-6 py-2 bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-600 transition shadow-sm disabled:opacity-50 text-sm"
								>
									{addingSource ? 'Adding...' : 'Add Document'}
								</button>
							</div>
						</div>
					</div>
				...
				<UnsavedChangesModal
					...
					onSave={async () => {
						const saved = await handleSaveTextDocument()
						if (saved) {
